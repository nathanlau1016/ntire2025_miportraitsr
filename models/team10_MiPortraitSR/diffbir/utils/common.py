...
from ..inference.pretrained_models import MODELS
import sys
...
def load_model_from_url(url: str) -> Dict[str, torch.Tensor]:
    if url == MODELS["sd_v2.1_zsnr"] or url == MODELS["sd_v2.1"]:
        sd_path = load_file_from_url(url, model_dir="weights/sd-2-1-base")
    else:
...
def load_model_from_url(url: str, model_path) -> Dict[str, torch.Tensor]:
    sd_path = load_file_from_url(url, model_dir=model_path)
    sd = torch.load(sd_path, map_location="cpu")
    if "state_dict" in sd:
        sd = sd["state_dict"]
    if list(sd.keys())[0].startswith("module"):
